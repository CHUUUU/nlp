...
if __name__ == "__main__":
    ...
    train_data_loader = DataLoader(train_data, batch_size=16, shuffle=True, num_workers=4, drop_last=True) # , collate_fn=train_data.collate)
    ...
    test_data_loader = DataLoader(test_data, batch_size=16, shuffle=True, num_workers=4, drop_last=True) #, collate_fn=test_data.collate)
    ...
    step = 0
    for i in range(5):
        for n, (ko_enc, en_dec, en_tar) in enumerate(train_data_loader):
            optimizer.zero_grad()
            logit = model(ko_enc.cuda(), en_dec.cuda())

            # batch flat
            en_tar = en_tar.view(-1)

            loss = loss_function(logit, en_tar.cuda())
            loss.backward()
            optimizer.step()
            ...
            if step % 100 == 0:
                ...
                total = 0
                correct = 0
                for n, (ko_enc, en_dec, en_tar) in enumerate(test_data_loader):
                    out = model(ko_enc.cuda(), en_dec.cuda())

                    _, pred = torch.max(out.data, 1)

                    total += 16*186
                    en_tar = en_tar.view(-1)
                    correct += (pred == en_tar.cuda()).sum()
                    if n == 0:
                        print(en_spm.decode(pred.view(16, -1)[0].tolist()))
